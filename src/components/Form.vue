...
import Column from 'primevue/column';
  ...
  const submitForm = () => {
    ...
    validateGender(true);
    if (!errors.value.username && !errors.value.password && !errors.value.gender && !errors.value.reason && !errors.value.age) {
  ...
  const validateGender = (blur) => {
    if (formData.value.gender === "male" || formData.value.gender ==="female" 
    || formData.value.gender === "other") {
  ...
  const validateReason = (blur) => {
    if (formData.value.reason.length < 10) {
        if(blur) errors.value.reason = "You must enter at least 10 characters";
    } else if (formData.value.reason.length > 100) {
        if(blur) errors.value.reason = "The reason should be summarized in less than 100 characters";
    } else {
